...
CREATE TABLE `staff_request` (
    ...
    `status` ENUM('WAITING', 'ACCEPTED', 'REFUSED') NOT NULL DEFAULT 'WAITING',
...
-- AddForeignKey
ALTER TABLE `staff_request` ADD CONSTRAINT `staff_request_id_user_fkey` FOREIGN KEY (`id_user`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
